            ...
            try {
                ...
                if (!handled) {
                    ...
                    if (matchedProduct) {
                        ...
                        await ctx.reply(`🛒 *Detail Layanan*\nNama: ${matchedProduct.product_name}\nBrand: ${matchedProduct.brand}\nKategori: ${matchedProduct.category}\n\n${matchedProduct.brand.toLowerCase().includes("by.u") || matchedProduct.brand.toLowerCase().includes("byu") ? "✏️ *Silakan masukkan Nomor HP by.U (atau Kode Pembayaran langsung):*\n\n_(Sistem akan mencarikan Kode Pembayaran otomatis)_" : (matchedProduct.brand.toLowerCase().includes("omni") || matchedProduct.brand.toLowerCase().includes("telkomsel omni") ? "✏️ *Silakan masukkan KODE PEMBAYARAN Telkomsel Omni (BUKAN Nomor HP):*" : "✏️ *Silakan masukkan nomor tujuan/pelanggan untuk mengecek tagihan:*")}`, {
                            parse_mode: 'Markdown',
                            reply_markup: {
                                keyboard: [[{ text: "❌ Batal" }]],
                                resize_keyboard: true
