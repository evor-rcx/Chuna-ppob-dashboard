cat << 'INNEREOF' > src/components/views/Produk.tsx
...
export function Produk({ onBack }: { onBack: () => void }) {
    ...
    try {
      ...
      if (data.success) {
        ...
        data.data.forEach((p: any) => {
           ...
           newHargas[p.buyer_sku_code] = { owner: (price + feeOwner).toString() };
        });
        setFees(newFees);
        setHargas(newHargas);
      } else {
  ...
  const handleSaveFee = async (sku: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) {
        alert(data.error || 'Gagal menyimpan fee');
      }
    } catch (err) {
      alert('Terjadi kesalahan saat menyimpan fee');
    } finally {
      setSavingSku(null);
    }
  };
