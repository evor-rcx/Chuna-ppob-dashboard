...
# I will replace the /api/members/:id/reset-pin endpoint.
old_endpoint = r"""  app\.post\("/api/members/:id/reset-pin", \(req, res\) => \{
    const \{ id \} = req\.params;
...
new_endpoint = """  app.post("/api/members/:id/reset-pin", async (req, res) => {
    const { id } = req.params;
    const memberIndex = members.findIndex(m => m.id === id);
    if (memberIndex !== -1) {
      const userId = id.replace('MBR-', '');
...
code = re.sub(old_endpoint, new_endpoint, code, flags=re.DOTALL)

with open('server.ts', 'w') as f:
    f.write(code)

print("Updated /api/members/:id/reset-pin to ask user for new PIN")
