...
pattern = r"""                    msg = `❌ Maaf Kak, pembayaran untuk pesanan Kakak gagal diproses\.Mungkin ada kesalahan data atau saldo kurang\. Coba cek lagi ya, atau hubungi Chuna untuk bantuan\.Keterangan: \$\{payJson\.data\.message \|\| 'Dibatalkan'\}📦 Tagihan: \$\{stateData\.product\.product_name\}🎯 Tujuan: \$\{customerNo\} \(\$\{checkResult\.customer_name \|\| "-"\}\)\$\{refundMsg\}Jangan khawatir, Kakak bisa coba ulang kapan saja\.Kalau butuh bantuan, chat Chuna di Bot Telegram:👉 https://t\.me/Chuna_Chan_botChuna siap bantu dengan senyum! 😊💪`;"""
...
if count > 0:
    print("Success replacing code in PASCA.")
else:
    print("Could not find old code in PASCA.")
